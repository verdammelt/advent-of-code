...
(defun coord-touching (c1 c2)
  (or (aoc:coord-equal c1 c2)
      (and (>= 1 (abs (- (aoc:coord-x c1) (aoc:coord-x c2))))
           (>= 1 (abs (- (aoc:coord-y c1) (aoc:coord-y c2)))))))

(defun up (coord) (aoc:make-coord (1+ (aoc:coord-x coord))
                              (aoc:coord-y coord)))
(defun down (coord) (aoc:make-coord (1- (aoc:coord-x coord))
                                (aoc:coord-y coord)))
(defun left (coord) (aoc:make-coord (aoc:coord-x coord)
                                (1- (aoc:coord-y coord))))
(defun right (coord) (aoc:make-coord (aoc:coord-x coord)
                                 (1+ (aoc:coord-y coord))))

(defun make-n-knot-rope (num-knots)
  (make-list num-knots :initial-element (aoc:make-coord 0 0)))
...
(defun move-follower (follower new-head)
  (let* ((l/r (if (> (aoc:coord-y follower) (aoc:coord-y new-head)) #'left #'right))
         (u/d (if (> (aoc:coord-x follower) (aoc:coord-x new-head)) #'down #'up)))
    (cond ((coord-touching follower new-head) follower)
          ((= (aoc:coord-x follower) (aoc:coord-x new-head)) (funcall l/r follower))
          ((= (aoc:coord-y follower) (aoc:coord-y new-head)) (funcall u/d follower))
