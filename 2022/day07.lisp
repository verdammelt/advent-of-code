...
(defgeneric order-nodes (fs-node1 fs-node2))
(defmethod order-nodes ((fs-node1 fs-node) (fs-node2 fs-node))
...
(defmethod print-object ((fs-node dir-node) stream)
  (print-unreadable-object (fs-node stream)
    (format stream "DIR: ~A: ~A" (name fs-node) (sort (copy-seq (contents fs-node)) #'order-nodes))))
